...
Voice-to-voice training simulator for getting past corporate front desks. You just speak (hands-free, automatic turn-taking); a harsh, realistic receptionist answers back in a real voice; your delivery is measured; a coach tears the transcript apart afterwards.
...
- Fully hands-free: a pause of about 1.4 seconds ends your turn, she answers out loud, and the mic re-opens when she finishes.
...
2. Paste your Anthropic API key and (optionally) ElevenLabs API key. Keys are stored in your browser's localStorage only and sent directly to the respective APIs. There is no backend.
3. Load voices, pick the most clipped businesslike one, pick a lobby and difficulty, hit Start conversation, and just talk.

If the mic is blocked when opening as a local file, serve it with: python3 -m http.server

## Costs

Per 10-minute session: a cent or two of Claude API usage and roughly 2k ElevenLabs credits. Browser TTS fallback is free.

## Privacy

No server, no analytics. Audio never leaves your machine except that Chrome's speech recognition uses Google's service for transcription, transcript text goes to Anthropic, and her lines go to ElevenLabs.
